...
//            Negative responses
            ...
            negative["I feel Like quitting"]?.get(1) -> "As expected bro. But just know no one will believe in you unless you believe in yourself. You're posting and there are no results, you're outreaching and no replies. That's what the hard is. It's all expected. You just focus on the goals hat aren't dependent on external entities. For example reach 100 posts, go to the gym, love your family, outreach to 100 people. Those are your reps"
            negative["I feel Like quitting"]?.get(2) -> "I feel you bro. The truth is that feeling never goes entirely. But soon you'll have a family to love. Just look at this. She loves you wholistically and she's from God bro. Just keep your eyes on God"
            
            negative["I feel lost and scared"]?.get(0) -> "When you start something new, you compare yourself to the established entities in that discipline. You're new so you feel out of place. when you start something new it makes sense you have no idea what's brackin' bro. This is where we build the self reliance. It makes you sufficient to weather any storm that you ever enter."
            negative["I feel lost and scared"]?.get(1) -> "Thats a lie from Satan! Dude God says to Joshua when Moses died 'Have I not commanded you? Be strong and of good courage; do no be afraid, nor be dismayed, for the LORD your God is with you wherever you go' - Joshua 1:9. Focus on God's Kingdom and meditate on the wisdom of being a Godly man"
            negative["I feel lost and scared"]?.get(2) -> "We don't need anyone!! You literally bought that phone by yourself. You prayed to God and the solar was installed. He's your support and your first love. Only pray and rely on Him, worship Him and Honour Him"
            
            negative["I feel alone"]?.get(0) -> "Thats a lie from Satan! Dude God says to Joshua when Moses died 'Have I not commanded you? Be strong and of good courage; do no be afraid, nor be dismayed, for the LORD your God is with you wherever you go' - Joshua 1:9. Focus on God's Kingdom and meditate on the wisdom of being a Godly man"
            negative["I feel alone"]?.get(1) -> "That makes sense considering how we were raised. But here we are going to look past that and understand that we are supposed to chase failure inorder to learn. That's where growth comes from. Remember how we sucked at 2048 then we just accepted failure and just tried to best the previous score. That's what you should aim to do. Just try best your last achievement. You got this bro trust!"
            negative["I feel alone"]?.get(2) -> "Remember in Philipians 4:6 says 'Be anxious for nothing, but in everything by prayer and supplication, with thanksgiving, let your requests be made known to God; and the peace that surpasses all understanding, will guard your hearts and minds through Christ Jesus. Stop trying to live for man, you asked for assistance and they just ignored you. Let the world be and don't live for them. You will come out alive and stronger and a pillar for your little angels"
            
            negative["I feel far from God"]?.get(0) -> "This Lord will never forsake you, it's you drawing away because you lack faith. In this moment just focus on Him and His promises. Just take 30 minutes off work and meditate on the Lord's promises. He's been with us since birth, when we were supposed to die but we didn't and He's still with us today. "
...
//Negative inputs
        ...
        }else if(message.trim().lowercase() == "i feel lost and scared"){
            val responses = negative["I feel lost and scared"]
            responses?.forEach { response ->
                val button = Button(this)
                button.text = response
                buttonContainer.addView(button)
        ...
        }else if(message.trim().lowercase() == "i feel alone"){
            val responses = negative["I feel alone"]
            responses?.forEach { response ->
                val button = Button(this)
                button.text = response
                buttonContainer.addView(button)
        ...
        }else if(message.trim().lowercase() == "i feel far from god"){
            val responses = negative["I feel far from God"]
            responses?.forEach { response ->
                val button = Button(this)
                button.text = response
                buttonContainer.addView(button)
        ...
        adapter.addMessage(messi)
        lifecycleScope.launch{
            delay(3000)
            messi.text = "Dude...you're doing more than anyone else trust me. Also don't forget who you're doing this for....MOM. We cant fail her. Also we need to look out for ourselves bro"
            adapter.notifyDataSetChanged()
        }
        adapter.addMessage(Message(null, false, null,videoResId = R.raw.lv_0_20250829142247, null, false))
            buttonContainer.removeAllViews()
            inputLayout.visibility = View.VISIBLE
        }else if(message.trim().lowercase() == "pressure from family"){
	        adapter.addMessage(Message(null, false, audioResId=R.raw.family_pressure))
            buttonContainer.removeAllViews()
            inputLayout.visibility = View.VISIBLE
        }else if(message.trim().lowercase() == "i don't feel like i'm enough"){
	        adapter.addMessage(Message("Look at your lovely family", false, imageResId = R.drawable.my_image13))
	        adapter.addMessage(Message(null, false, audioResId=R.raw.im_not_enough))
	        adapter.addMessage(Message("Look at your bride", false, imageResId= R.drawable.my_image_12))
            buttonContainer.removeAllViews()
            inputLayout.visibility = View.VISIBLE
        }else if(message.trim().lowercase() == "there are no results"){
		        adapter.addMessage(Message(null, false, audioResId=R.raw.no_results))
		        adapter.addMessage(Message("Soon there will be lol", false, imageResId=R.drawable.my_image2))
                buttonContainer.removeAllViews()
                inputLayout.visibility = View.VISIBLE
    }else if(message.trim().lowercase() == "there's no support"){
			    adapter.addMessage(Message(null, false, audioResId=R.raw.no_support))
                buttonContainer.removeAllViews()
                inputLayout.visibility = View.VISIBLE
    }else if(message.trim().lowercase() == "no one cares"){
                buttonContainer.removeAllViews()
                inputLayout.visibility = View.VISIBLE
    }else if(message.trim().lowercase() == "i don't have any support"){
			    adapter.addMessage(Message("Never needed it", false, imageResId=R.drawable.my_image6))
                buttonContainer.removeAllViews()
                inputLayout.visibility = View.VISIBLE
    }else if(message.trim().lowercase() == "burden of failure feels heavy"){
			    adapter.addMessage(Message(null, false, audioResId=R.raw.failure))
                buttonContainer.removeAllViews()
                inputLayout.visibility = View.VISIBLE
    }else if(message.trim().lowercase() == "no one to confide in"){
			    adapter.addMessage(Message(null, false, imageResId=R.drawable.my_image4))
                buttonContainer.removeAllViews()
                inputLayout.visibility = View.VISIBLE
    }else if(message.trim().lowercase() == "nothing is working out"){
			    adapter.addMessage(Message(null, false, audioResId=R.raw.far_from_god))
                buttonContainer.removeAllViews()
                inputLayout.visibility = View.VISIBLE
    }else {
